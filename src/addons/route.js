...
mag.route = function() {
  ...
  //m.route(el, defaultRoute, routes)
    ...
    redirect = function(source) {
      ...
      if (!routeByValue(root, router, path)) {
        if (isDefaultRoute)
          throw new Error(
            'Ensure the default route matches one of the routes defined in mag.route'
          );
        isDefaultRoute = true;
  ...
  //config: m.route
    ...
    var ohref = context._href
      ? context._href
      : (context._href = element.getAttribute('href'));

    element.href =
      (mag.route.mode !== 'pathname' ? $location.pathname : '') +
      modes[mag.route.mode] +
      ohref;
  ...
  //m.route(route, params, shouldReplaceHistoryEntry)
    ...
    var params =
      queryIndex > -1
        ? parseQueryString(currentRoute.slice(queryIndex + 1))
        : {};
    for (var i in args) params[i] = args[i];
    var querystring = buildQueryString(params);
    var currentPath =
      queryIndex > -1 ? currentRoute.slice(0, queryIndex) : currentRoute;
    if (querystring)
      currentRoute =
        currentPath +
    ...
    var shouldReplaceHistoryEntry =
      (arguments.length === 3 ? arguments[2] : arguments[1]) === true ||
    ...
    if (window.history.pushState) {
      ...
      computePostRedrawHook = function() {
        window.history[
          shouldReplaceHistoryEntry ? 'replaceState' : 'pushState'
        ](null, $document.title, modes[mag.route.mode] + currentRoute);
      };
    ...
    if (matcher.test(path)) {
      path.replace(matcher, function() {
        ...
        for (var i = 0, len = keys.length; i < len; i++)
          routeParams[keys[i].replace(/:|\./g, '')] = decodeURIComponent(
            values[i]
          );
        router[route]();
        //console.log(path, root, routeParams)
...
function routeUnobtrusive(e) {
  ...
  var args =
    mag.route.mode === 'pathname' && currentTarget.search
  ...
  mag.route(
    currentTarget[mag.route.mode].slice(modes[mag.route.mode].length),
    args
  );
}

function setScroll() {
  if (mag.route.mode != 'hash' && $location.hash)
