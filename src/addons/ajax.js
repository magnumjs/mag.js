...
(function(window, document, mag, undefined) {
  ...
  function ajax(options) {
    if (options.dataType && options.dataType.toLowerCase() === 'jsonp') {
      ...
      if (
        data &&
        (type.call(data) != STRING && data.constructor != window.FormData)
      ) {
  ...
  mag.request = function(xhrOptions) {
    ...
    var isJSONP =
      xhrOptions.dataType && xhrOptions.dataType.toLowerCase() === 'jsonp';
    var serialize = (xhrOptions.serialize = isJSONP
      ? identity
      : xhrOptions.serialize || JSON.stringify);
    var deserialize = (xhrOptions.deserialize = isJSONP
      ? identity
      : xhrOptions.deserialize || JSON.parse);
    var extract = isJSONP
      ? function(jsonp) {
    ...
    xhrOptions.onload = xhrOptions.onerror = function(e) {
      try {
        ...
        var unwrap =
          (e.type === 'load'
            ? xhrOptions.unwrapSuccess
            : xhrOptions.unwrapError) || identity;

        var response = unwrap(
          deserialize(extract(e.target, xhrOptions)),
          e.target
        );
