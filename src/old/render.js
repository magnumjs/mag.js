(function(mag) {
  ...
  render.callLCEvent = function(eventName, module, index, once) {
    ...
    if (module.controllers[index] && module.controllers[index][eventName]) {
      module.controllers[index][eventName].call(
        {},
        event,
        module.elements[index]
      );
  ...
  function addConfigUnloaders(module, fill, index) {
    ...
    for (var k in fill.cached) {
      ...
      if (
        k.indexOf('id("' + module.elements[index].id + '")/') !== -1 &&
        k.indexOf('-config') !== -1 &&
        fill.cached[k].configContext
      ) {
  ...
  var throttle = function(fn, threshhold) {
    var lastRedrawCallTime,
      FRAME_BUDGET = threshhold || 16,
      deferTimer;
    return function() {
      var args = arguments;
      if (
    ...
    var observer = function(changes) {
      changes.forEach(function(change) {
        if (
          change.type == 'update' &&
          change.oldValue &&
          change.oldValue.type == 'fun' &&
          change.oldValue.data &&
          change.oldValue.data.type == 'module' &&
      ...
      throttle(
        render.doWatch.bind({}, fill, elementClone, i, module, changes)
      )();
    };
  ...
  function notifySubobjectChanges(object) {
    ...
    var handle = function(changes) {
      // observe the property value
      changes.forEach(function(change) {
        // and for each change
