(function(mag, document, undefined) {
  ...
  mag.prop = function(store, custom) {
    if (
      ((store != null && type.call(store) === OBJECT) ||
  ...
  function runPromise(index, controller, clone) {
    ...
    module.promises[index] = new Promise(
  ...
  function getRendVal(domElementId, index, clone, props) {
    ...
    */
    ...
    if (index < 0 && (clone || typeof props.key == 'undefined')) {
      props.key =
        typeof props.key != 'undefined' ? props.key : render.roots.length;
    }

    return typeof props.key != 'undefined'
      ? domElementId + '.' + props.key
      : domElementId;
  }
  ...
  mag.module = function(domElementId, moduleObject, props, clone) {
    //MODULE
    if (!moduleObject.view)
      throw Error(
        'Mag.JS module - requires a view: ' + domElementId + moduleObject
      );
...
*/
    ...
    if (render.roots.indexOf(rendVal) < 0) {
      ...
      if (!element)
        throw Error('Mag.JS Module - invalid node id: ' + domElementId);
