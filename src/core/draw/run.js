...
const run = function(cloner, id, props2, module, clear) {
  ...
  if (exists(ids)) {
    if (callLCEvent('willgetprops', getState(ids), cloner, ids, 0, props2))
      return;
  }
...
var willloader = function(idInstance, node) {
  return callLCEvent('willload', getState(idInstance), node, idInstance, 1);
};
...
var didloader = function(idInstance, node) {
  ...
  if (getNode(id) && exists(idInstance)) {
    return callLCEvent('didload', getState(idInstance), node, idInstance, 1);
  }
};
