...
function getPathTo2(element, cached) {
  ...
  if (element === mag.doc.body) return element.tagName;
...
function addToNode(node, val) {
  ...
  if (
    ...
    (val.id && !mag.doc.getElementById(val.id)) ||
...
function fillNode(node, data, p, key) {
  ...
  if (
    mag.mod.innerMods[fill.id] &&
    data[mag.mod.innerMods[fill.id][0]] &&
    !data[mag.mod.innerMods[fill.id][0]].draw
  ) {
