...
var funRet = mod => props => (index, p) => {
  props = props || {};
  props.key = (props.key ? index + '.' + props.key : index) + p;
  return mod(props);
};
...
mag.create = function(idOrNode, moduleDefinition, defaultProps) {
  if (typeof moduleDefinition == 'function') {
    ...
    return funRet(mod);
  } else if (typeof idOrNode == 'function') {
    // mag
    ...
    return (id2, props2) => {
      ...
      return mag.module(
        id2 || idOrNode,
        moduleDefinition,
        mag.merge(mag.copy(defaultProps) || {}, mag.copy(props2) || {})
      );
    };
  }
};
...
mag.template = function(url, cb) {
  ...
  return mag.request(req).then(function(data) {
...
mag.cache = function(key, data, cacheTime, storageType) {
  ...
  if (mag.cache.data[key] && mag.cache.data[key].id)
    clearTimeout(mag.cache.data[key].id);
    ...
    forEach(data.value, function(key, value) {
      if (value != null) {
        nstyle +=
          camelDash(key) +
          ':' +
          (!isNaN(value) && !(key in unitlessProps) ? value + 'px' : value) +
          ';';
      }
    });
...
hookin('attributes', 'className', function(data) {
  ...
  if (typeof data.value == 'string') {
    ...
    for (var k in newClass) {
      data.node.classList.toggle(
        k,
        typeof newClass[k] !== 'undefined' && newClass[k]
      );
    }
...
hookin('values', '*', function(data) {
  ...
  else if (dtype === 'function' && data.value.toJSON) {
    data.change = 1;
    data.value = data.value();
  }
  // Allow for promises to be resolved
  else if (dtype == 'object' && typeof data.value.then == 'function') {
